...
public class Sphere extends Mesh {
    ...
    public Sphere(final String name, final int zSamples, final int radialSamples, final double radius) {
        this(name, new Vector3D(0, 0, 0), zSamples, radialSamples, radius, Material.getShinyMaterial());
    }
     ...
     */
    public Sphere(final String name, final Vector3D center, final int zSamples, final int radialSamples,
    ...
    private void setGeometryData() {
        ...
        for (int iZ = 1; iZ < (zSamples - 1); iZ++) {
            ...
            kSliceCenter = new Vector3D(center.getX(), center.getY(), center.getZ() + fZ);
            ...
            for (int iR = 0; iR < radialSamples; iR++) {
                ...
                tempVa= new Vector3D(fSliceRadius*kRadial.getX(),fSliceRadius*kRadial.getY(),fSliceRadius*kRadial.getZ());
                ...
                kNormal = new Vector3D(tempVa.getX()-center.getX(),tempVa.getY()-center.getY(),tempVa.getZ()-center.getZ());
                double mag = Math.sqrt(Math.pow(kNormal.getX(),2)+Math.pow(kNormal.getY(),2)+Math.pow(kNormal.getZ(),2));
                kNormal = new Vector3D(kNormal.getX()/mag, kNormal.getY()/mag, kNormal.getZ()/mag);
                if (!viewInside) {
...
//            setNormalCoord(i, getNormalCoord(iSave));
        ...
        // south pole
//        setVertexCount(i+1);
        putVertex(new Vector3D(center.getX(),center.getY(),(float) (center.getZ() - radius)));
...
//        setNormalCount(i+1);
    ...
    private void setIndexData() {
        ...
        for (int i = 0; i < radialSamples; i++, index += 3) {
            if (!viewInside) {
                putTriangleIndex(i + iOffset, i + 1 + iOffset, getVertexCount() - 1);
            } else // inside view
            {
                putTriangleIndex(i + iOffset, getVertexCount() - 1, i + 1 + iOffset);
            }
        }
    }
