...
//		final Rod3D[] bordersR;
    ...
    public Polygon3D(Vector3D[] O, boolean debug) {
        ...
        for ( int i = 0; i < NUM; i++ ) {
            vertices[i] = Vector3D.rotateAngleAxis (Vector3D.subtract (O[i],
                    O[0]), R);
            pX = vertices[i].getX ();
            pY = vertices[i].getY ();
...
//					bordersR[i-1] = new Rod3D(P[i-1].getX(), P[i-1].getY(), P[i].getX(), P[i].getY());
                borders[i - 1] = new Line (P[i - 1].getX (), P[i - 1].getY (),
                        P[i].getX (), P[i].getY ());
                borders[i - 1].setStrokeWidth (0.2);
            }
            if ( i == last ) {
//					bordersR[i] = new Rod3D(P[i].getX(), P[i].getY(), P[0].getX(), P[0].getY());
                borders[i] = new Line (P[i].getX (), P[i].getY (), P[0].getX (),
                        P[0].getY ());
            ...
            if ( debug ) {
                if ( i == 0 ) {
                    P1 = new Polygon (pX, pY, pX, pY, pX, pY);
                }
                if ( i == 1 ) {
                    P2 = new Polygon (pX, pY, pX, pY, pX, pY);
                }
                if ( i == 2 ) {
                    P3 = new Polygon (pX, pY, pX, pY, pX, pY);
                }
        ...
        centroidNormal.getTransforms ().add (new Translate (ct.getX (), ct.
                getY (), ct.getZ ()));

        centroidNormal.getTransforms ().add (
                new Scale (normalScale, normalScale, 40 * normalScale));
        ...
        Rotate r1 = new Rotate (Vector3D.radToDeg (-Rangle), 0, 0, 0, Raxis.
                toPoint3D ());
        ...
        if ( debug ) {
            ...
            if ( debug ) {
                System.out.println (" O1=" + O[0] + " O2=" + O[1] + " O3=" +
                        O[2]);
...
//System.out.println("normalCoord0=" + normalCoord0 + "  normalCoord1=" + normalCoord1 + "  normalCoord2=" + normalCoord2);
        ...
        for ( int i = 0; i < NUM; i++ ) {
            rods[i] = new Rod3D (O[i], Vector3D.add (O[i], Vector3D.multiply (
                    normalCoords[i], 5)), Color.MAGENTA, 0.25);
...
//		getChildren().add(decoration);
    ...
    public void setText(String s) {
        ...
        double width = text.getBoundsInLocal ().getMaxX () - text.
    ...
    public double calculateDiffuse(Vector3D normalCoord, Vector3D lightVector,
            double Kd) {
        //double theta = Vector3D.angleBetween(normalCoord, lightVector);
        double dotP = Vector3D.dotProduct (normalCoord.normaliseThis (),
                lightVector.normaliseThis ());
...
//System.out.println("");
    ...
    public void calculateAverageGradientShade(Lighting3D lighting,
            Material material, Vector3D normalCoord0, Vector3D normalCoord1,
            Vector3D normalCoord2) {
...
//System.out.println("SHADES 0=" + calculateDiffuse(normalCoord0, lightVector));
                vColor[0] = vColor[0] + (calculateDiffuse (normalCoord0,
                        lightVector, diffuseIntensity));
                vColor[1] = vColor[1] + (calculateDiffuse (normalCoord1,
                        lightVector, diffuseIntensity));
                vColor[2] = vColor[2] + (calculateDiffuse (normalCoord2,
                        lightVector, diffuseIntensity));
            }
        }
...
// Length of triangle sides
        ...
        verticesFlatT[0]
                = new Point2D (pointsFlatT.get (0), pointsFlatT.get (1));
        verticesFlatT[1]
                = new Point2D (pointsFlatT.get (2), pointsFlatT.get (3));
        verticesFlatT[2]
                = new Point2D (pointsFlatT.get (4), pointsFlatT.get (5));

        Point3D[] verticesShadeT = new Point3D[3];
        verticesShadeT[0] = new Point3D (verticesFlatT[0].getX (),
                verticesFlatT[0].getY (), shades[0]);
        verticesShadeT[1] = new Point3D (verticesFlatT[1].getX (),
                verticesFlatT[1].getY (), shades[1]);
        verticesShadeT[2] = new Point3D (verticesFlatT[2].getX (),
                verticesFlatT[2].getY (), shades[2]);

        Point3D shadeV1 = null;
        Point3D shadeV2 = null;
        if ( maxShadeIndex == 0 ) {
            shadeV1 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[0],
                    verticesShadeT[1])).toPoint3D ();
            shadeV2 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[0],
                    verticesShadeT[2])).toPoint3D ();
        }
        if ( maxShadeIndex == 1 ) {
            shadeV1 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[1],
                    verticesShadeT[2])).toPoint3D ();
            shadeV2 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[1],
                    verticesShadeT[0])).toPoint3D ();
        }
        if ( maxShadeIndex == 2 ) {
            shadeV1 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[2],
                    verticesShadeT[0])).toPoint3D ();
            shadeV2 = Vector3D.normalise (Vector3D.getVector (verticesShadeT[2],
                    verticesShadeT[1])).toPoint3D ();
        }

//System.out.println("shadeV1=" + shadeV1);
//System.out.println("shadeV2=" + shadeV2);
        Vector3D normalT = Vector3D.normalise (Vector3D.crossProduct (
                new Vector3D (shadeV1), new Vector3D (shadeV2)));
...
//System.out.println("thetaDeg=" + thetaDeg);
        ...
        Vector3D rotAxis = Vector3D.normalise (Vector3D.crossProduct (normalUP,
                normalT));
...
//		Point2D gradStop = new Point2D(gradLength*fallAxis.getX()+verticesFlatT[maxShadeIndex].getX(), gradLength*fallAxis.getY()+verticesFlatT[maxShadeIndex].getY());
        double gradStopX = gradLength * fallAxisX +
                verticesFlatT[maxShadeIndex].getX ();
        double gradStopY = gradLength * fallAxisY +
                verticesFlatT[maxShadeIndex].getY ();
...
//System.out.println("gradColor=" + gradColor + "material.color=" + material.color + "maxShade=" + maxShade);
        ...
        Stop[] stopTred = new Stop[]{new Stop (0, new Color (gradColor.x,
            gradColor.y, gradColor.z, 1.0)), new Stop (1, new Color (0.0, 0.0,
            0.0, 1.0))};
        ...
        LinearGradient lgTred = new LinearGradient (
                verticesFlatT[maxShadeIndex].getX (),
                verticesFlatT[maxShadeIndex].getY (), gradStopX, gradStopY,
                false, CycleMethod.NO_CYCLE, stopTred);
...
//System.out.println("SHADES 0=" + calculateDiffuse(normalCoord0, lightVector));
                vColor[0] = vColor[0] + (calculateDiffuse (normal, lightVector,
                        diffuseIntensity));
            }
        }
    ...
    public final static Point3D getPointFromSingularBounds(Shape shape) {
        ...
        return new Point3D (shapeBounds.getMinX (), shapeBounds.getMinY (),
                shapeBounds.getMinZ ());
    }
