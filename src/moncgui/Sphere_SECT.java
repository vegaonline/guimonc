...
public class Sphere_SECT extends Mesh {
    ...
    private void setGeometryData() {
        ...
        for (int iR = 0; iR < radialSamples; iR++) {
            //  final double fAnglePhi = 2 * Math.PI * fInvRS * iR;
            final double fAnglePhi = phi0 + dPhi * fInvRS * iR;
            afCos[iR] = Math.cos(fAnglePhi);
            afSin[iR] = Math.sin(fAnglePhi);
            System.out.println(afCos[iR] + "  " + afSin[iR]);
        }
        afSin[radialSamples] = afSin[0];
        afCos[radialSamples] = afCos[0];
        System.out.println(afCos[radialSamples] + "  " + afSin[radialSamples]);
...
//        setNormalCount(i+1);
    ...
    /**
     *
     * @param viewInside if true, the normals are inverted to point into the sphere so that the face is oriented for a
     * viewer inside the sphere. Default is false (for outside viewing)
     */
