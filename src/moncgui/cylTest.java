...
/**
 * <code>cylTest</code> provides an extension of <code>Mesh</code>. A
 * <code>cylTest</code> is defined by a height and radius. The center of the
 * cylTest is the origin.
 */
/**
 *
 * @author vega
...
public class cylTest extends Mesh {
    ...
    /**
     * Creates a new cylTest. By default its center is the origin. Usually, a
     * higher sample number creates a better looking cylinder, but at the cost
     * of more vertex information.
     *
     * @param name The name of this cylTest.
     ...
     */
    public cylTest(final String name, final int axisSamples,
            final int radialSamples, final double radius,
            final double height) {
     ...
     * @param height The cylinder's height.
     * @param closed true to create a cylinder with top and bottom surface
     * @param inverted true to create a cylinder that is meant to be viewed from
     * the interior.
     */
    public cylTest(final String name, final int axisSamples,
            final int radialSamples, final double radius,
            final double height, final boolean closed, final boolean inverted) {
        this (name, axisSamples, radialSamples, radius, height, closed, inverted,
     ...
     * This makes the Mesh be a frustum of pyramid, or if set to 0, a cone.
     *
    ...
    private void allocateVertices() {
        ...
        final int count = ((closed ? 2 : 0) + 2 * (axisSamples - 1)) *
                radialSamples;
...
// System.out.println("");
    ...
    private void setGeometryData() {
        ...
        final double inverseAxisLess = 1.0 / (closed ? axisSamples - 3
                : axisSamples - 1);
        ...
        for ( int axisCount = 0, i = 0; axisCount < axisSamples; axisCount++ ) {
            ...
            final int save = i;
            for ( int radialCount = 0; radialCount < radialSamples;
                    radialCount++ ) {
                final double radialFraction = radialCount * inverseRadial; // in [0,1)
                tempNormal
                        = new Vector3D (cos[radialCount], sin[radialCount], 0);
                if ( topBottom == 0 ) {
                    if ( !inverted ) {
                        putNormal (tempNormal.getX (), tempNormal.getY (),
                                tempNormal.getZ ());
                    } else {
                        putNormal (-tempNormal.getX (), -tempNormal.getY (),
                                -tempNormal.getZ ());
                ...
                putVertex (tempNormal.getX () * factor + sliceCenter.getX (),
                        tempNormal.getY () * factor + sliceCenter.getY (),
                        tempNormal.getZ () * factor + sliceCenter.getZ ());
    ...
    private void setIndexData() {
        ...
        for ( int axisCount = 0, axisStart = 0; axisCount < axisSamples - 1;
                axisCount++ ) {
            int i0 = axisStart;
            int i1 = i0 + 1;
