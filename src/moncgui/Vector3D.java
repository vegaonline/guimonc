...
// STAIC methods returning value
	...
	public final static Vector3D normalise(Vector3D A){
		...
		if (mag==0) {
			...
			return new Vector3D(A.getX() / mag, A.getY() / mag, A.getZ() / mag);
		}
	}
...
// STATIC methods using result vector
	public final static void crossProduct (Vector3D A, Vector3D B, Vector3D result)  {
	    result.setX( A.getY() * B.getZ() - A.getZ() * B.getY());
	    result.setY( A.getZ() * B.getX() - A.getX() * B.getZ());
	    result.setZ( A.getX() * B.getY() - A.getY() * B.getX());
  	}
	public final static void normalVector(Vector3D A, Vector3D B, Vector3D result){
	...
	public final static void getVector(Vector3D A, Vector3D B, Vector3D result){
 		result.setXYZ(B.getX()-A.getX(), B.getY()-A.getY(), B.getZ()-A.getZ());
	}
...
*/
	public final static void getRotateAngleAxisMatrix(double theta, Vector3D axis, double[][] R){
	...
	public final static double[][] getRotateAngleAxisMatrix(double theta, Vector3D axis){
		double c = Math.cos(theta); //radians
		double t = 1 - c;
		double s = Math.sin(theta); //radians
		double x = axis.getX();
	...
	public final static Vector3D rotateAngleAxis(Vector3D v, double theta, Vector3D axis){
		double c = Math.cos(theta); //radians
		double t = 1 - c;
		double s = Math.sin(theta); //radians
		double x = axis.getX();
